...
- `dnf install` is right on Fedora, and not on Fedora CoreOS. If you don't have product-specific variants, the agent either stops with an error, or fails silently.
...
- Then the sentences that are not instructions at all: be careful, use your judgment, check with the team, it would be nice if you could add tests. Documentation produces the same sentences. *Consult your system administrator. Use caution when modifying this value.* I have written both. None of them can be followed or checked.
...
ETH Zurich found that adding a context file did not generally improve task success, and raised inference cost by more than 20% on average. They tested on SWE-bench tasks and on a benchmark of repositories that already had hand-written files, and the result held across models and agents, whether the file was hand-written or generated [@gloaguen2026].[^ethnuance] The same study's trace analysis shows how strong that compliance is: a tool got used far more often in the instances where the context file named it than in the instances where it didn't.
...
Every fault above has the same cause. The file has no lifecycle.
...
Underneath that is the part no tooling reaches. Owning the file earns nobody a ticket or a review credit, so it stays unowned. Blocking a merge over *use caution* costs the reviewer more than letting it through, so empty rules accumulate.

The faults also split in two, and the halves need different treatment. Dead paths, stale versions and wrong commands are mechanical: a script can check every path, command and version the file names, and it will still be checking them next year. Contradictions and empty rules are not. No tool reads *never force-push* against *force-pushing to your own feature branch is fine* and reports a conflict, and none will tell you *be careful* is not an instruction. Those need a person.
...
Instructions differ by version and by variant. 4.2 needs different guidance from 4.3. Fedora and Fedora CoreOS need different install commands, as above.
...
The two cheap rows buy most of the value. Deleting on a schedule costs nothing and fixes contradictions, dead paths and stale versions. For one repo, one target and two people, the first row is the whole answer. Generation is worth it at two targets and hard to justify at one, and I have not seen anyone run it for instruction files, so treat that row as a mechanism rather than a practice.
...
None of this is a documentation invention. Version control, code review, CI, conditional compilation: all of it came from engineering, and documentation adopted it because writing prose outside the repository had stopped working.

Documentation applied it to prose, which is a different job: you cannot compile a sentence or unit-test a paragraph. Keeping a large body of text true across versions and products is dull work that has been done before.
...
[^nested]: More precisely, at least for Codex: it concatenates every ancestor `AGENTS.md` from the project root down to the working directory, with the nearest file placed last so it overrides on conflict. A rule in the root file still applies anywhere a more specific file doesn't override it. Claude Code and Cursor follow a similar hierarchical pattern.
